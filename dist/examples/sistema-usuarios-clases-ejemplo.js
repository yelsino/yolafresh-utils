...
exports.inicializarSistemaUsuarios = inicializarSistemaUsuarios;
exports.crearEntidadesEjemplo = crearEntidadesEjemplo;
exports.crearRolesEjemplo = crearRolesEjemplo;
exports.ejemploCompletoConClases = ejemploCompletoConClases;
exports.ejemploClaseUsuario = ejemploClaseUsuario;
exports.ejemploSesionManager = ejemploSesionManager;
exports.ejemploPermisoValidator = ejemploPermisoValidator;
exports.ejecutarTodosLosEjemplos = ejecutarTodosLosEjemplos;
const Usuario_1 = require("../class/Usuario");
const UsuarioManager_1 = require("../class/UsuarioManager");
const SesionManager_1 = require("../class/SesionManager");
const PermisoValidator_1 = require("../class/PermisoValidator");
...
function inicializarSistemaUsuarios() {
    ...
    const usuarioManager = new UsuarioManager_1.UsuarioManager({
        auditoria: true,
        cache: true,
 ...
 */
function crearEntidadesEjemplo() {
    // Cliente ejemplo
    const cliente = {
        id: "cli_001",
        tipo: "Cliente",
        ...
        historialCompras: [],
        totalGastado: 0,
        categoria: persons_1.CategoriaCliente.VIP,
 ...
 */
function crearRolesEjemplo() {
    return {
        rolAdmin: (0, rbac_1.crearRolPredefinido)(entidades_1.RolesPredefinidos.ADMIN, "rol_admin_001"),
        rolCajero: (0, rbac_1.crearRolPredefinido)(entidades_1.RolesPredefinidos.CAJERO, "rol_cajero_001"),
        rolCliente: (0, rbac_1.crearRolPredefinido)(entidades_1.RolesPredefinidos.CLIENTE, "rol_cliente_001")
    };
}
// ========================================
 ...
 */
async function ejemploCompletoConClases() {
    console.log("\n" + "=".repeat(60));
    ...
    try {
        ...
        const resultadoAdmin = await usuarioManager.crearUsuario(datosAdmin, [rolAdmin], [] // Admin no necesita entidades específicas
        );
        ...
        const resultadoCajero = await usuarioManager.crearUsuario(datosCajero, [rolCajero], [personal], usuarioAdmin // Creado por el admin
        );
        ...
        const resultadoCliente = await usuarioManager.crearUsuario(datosCliente, [rolCliente], [cliente], usuarioAdmin);
        ...
        console.log(`   ✅ Cajero puede crear ventas: ${usuarioCajero.puede(entidades_1.Permisos.VENTAS_CREAR)}`);
        console.log(`   ❌ Cliente puede crear ventas: ${usuarioCliente.puede(entidades_1.Permisos.VENTAS_CREAR)}`);
        // Permisos múltiples
...
function ejemploClaseUsuario() {
    ...
    console.log(`✅ Puede crear ventas: ${usuario.puede(entidades_1.Permisos.VENTAS_CREAR)}`);
 ...
 */
async function ejemploSesionManager() {
    console.log("\n🎪 EJEMPLO: SesionManager");
    console.log("-".repeat(40));
    const sesionManager = new SesionManager_1.SesionManager({
    ...
    try {
        ...
        const validarPassword = async (password, hash) => {
 ...
 */
async function ejemploPermisoValidator() {
    console.log("\n✅ EJEMPLO: PermisoValidator");
    console.log("-".repeat(40));
    const validator = new PermisoValidator_1.PermisoValidator();
    const { cliente, personal } = crearEntidadesEjemplo();
    const { rolCajero, rolCliente } = crearRolesEjemplo();
    const usuarioCajero = new Usuario_1.Usuario({
    ...
    const usuarioCliente = new Usuario_1.Usuario({
    ...
    const contextoCajero = { usuario: usuarioCajero };
    const contextoCliente = { usuario: usuarioCliente };
