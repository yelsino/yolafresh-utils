...
export function validarRutasPreparacionRestaurante(
  ...
  const errors: string[] = [];
  if (rutas.length === 0) errors.push("El producto debe tener al menos una ruta de preparacion");
  const ids = new Set<string>();
  for (const ruta of rutas) {
    if (!ruta.estacionPreparacionId.trim()) errors.push("La ruta requiere una estacion");
    if (ids.has(ruta.estacionPreparacionId)) errors.push("La estacion no puede repetirse en las rutas");
...
export function validarProductoRestaurante(
  ...
  const errors = [...validarRutasPreparacionRestaurante(
