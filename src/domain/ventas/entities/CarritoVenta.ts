...
import { Presentacion, TipoVentaEnum } from "../../inventario/contracts/producto.contract";
...
type CarritoVentaProductInput = Partial<Presentacion> & {
  descripcion?: string;
  titulo?: string;
};

type CarritoVentaItemWithMetadata = CarItem & {
  displayName?: string;
...
export class CarritoVenta implements ICarritoVenta {
  ...
  toJSON() {
    const carrito: ICarritoVenta = {
      ...
      items: this._items.map(item => {
        const product = item.product as CarritoVentaProductInput;
  ...
  toVentaSnapshot(): CarritoVentaSnapshot {
    ...
    const items = this._items.map((item) => {
      ...
      const precioVenta = Number(item.product?.precioVenta ?? 0);
      ...
      const itemMetadata = item as CarritoVentaItemWithMetadata;
      return {
        id: item.id,
        product,
        quantity,
